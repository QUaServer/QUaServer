...
!build_pass {
	...
	win32 {
		...
		system("cmake $${OPEN62541_PATH_WIN} -B$${OPEN62541_BUILD_PATH_WIN} -DUA_ENABLE_AMALGAMATION=ON $${UA_NAMESPACE} $${UA_EVENTS} $${UA_HISTORIZING} -G \"$${COMPILER}\" -A $${PLATFORM}"): PROJECT_CREATED = TRUE
		equals(BUILD_CREATED, TRUE) {
	...
	linux-g++ {
		...
		# Generate CMake project
		PROJECT_CREATED = FALSE
		system("cmake $${OPEN62541_PATH} -B$${OPEN62541_BUILD_PATH} -DUA_ENABLE_AMALGAMATION=ON $${UA_NAMESPACE} $${UA_EVENTS} $${UA_HISTORIZING}"): PROJECT_CREATED = TRUE
		equals(BUILD_CREATED, TRUE) {
			message("CMake generate open62541 successful.")
		}
		else {
